...
fn main() {
    ...
    build
        .file("src/cpp/nam_wrapper.cpp")
        .includes(&[
            env::current_dir().unwrap().join("src"),
            env::current_dir().unwrap().join("NeuralAmpModelerCore"),
            env::current_dir().unwrap().join("NeuralAmpModelerCore").join("NAM"),
            env::current_dir().unwrap().join("NeuralAmpModelerCore").join("Dependencies").join("eigen"),
            env::current_dir().unwrap().join("NeuralAmpModelerCore").join("Dependencies").join("nlohmann"),
        ]);
